...
"""
content = content.replace('<section class="tab-pane" id="tab3">', tab2_html + '\n            <section class="tab-pane" id="tab3">')

# 5. PB Cockpit row-1 replacement
old_row1_match = re.search(r'<div class="cockpit-row-1">.*?</div>\s*</div>\s*<!-- 2단계', content, re.DOTALL)
if old_row1_match:
    old_row1 = old_row1_match.group(0)
    new_row1 = """<div class="cockpit-row-1">
                            <!-- 인적 요약 정보 -->
...
old_buttons_match = re.search(r'<button class="btn-large" id="defendBtn" onclick="runDefendMagnet\(\)">🚀 마그넷 솔루션 가동 \(방어 실행\)</button>', content)
if old_buttons_match:
    new_buttons = """<div style="display:flex; gap:10px; width:100%;">
...
# I will append a script block at the bottom just before </body> to override the original selectCustomer and add runRollover logic.
script_append = """
    <script>
...
"""
content = content.replace('</body>', script_append)
