...
def load_combined_data(api_data_len=0, serialized_api_data=None):
    ...
    # 지능적인 데이터 경로 탐색 (루트 및 testproj_1 폴더 고려)
    data_dir = os.path.join(base_dir, "data")
    if not os.path.exists(data_dir):
        data_dir = os.path.join(base_dir, "testproj_1", "data")
    ...
    if csv_file is None or not os.path.exists(csv_file):
        csv_file = os.path.join(data_dir, "식품의약품안전처_건강기능식품영양성분정보_20251230.csv")
    ...
    # --- 2단계: 수집 완료된 전체 데이터 CSV 또는 JSON 파일 로드 및 파싱 (국내건기식) ---
    rows = []
    ...
    if serialized_api_data is not None:
        ...
        # 실시간 호출이 없으면, 4.5만 건 전체 동기화 완료된 로컬 CSV 파일 먼저 로드
        total_csv_file = os.path.join(base_dir, "nutrifit_total_data.csv")
        if not os.path.exists(total_csv_file):
            total_csv_file = os.path.join(os.path.dirname(base_dir), "nutrifit_total_data.csv")
            
        if os.path.exists(total_csv_file):
            try:
                # pandas로 불러온 뒤 C003 JSON row 형식과 맞추기 위해 dictionary list로 변환
                df_total = pd.read_csv(total_csv_file, encoding="utf-8-sig")
                df_total = df_total.fillna("")
                rows = df_total.to_dict(orient="records")
            except Exception as e:
                st.error(f"전체 동기화 CSV 로딩 실패: {str(e)}")
        else:
            # 전체 동기화 파일이 없는 경우 기존 백업 JSON 파일 로드
...
<div class="card" style="margin-top:25px;">
    ...
    with col_t1_b2:
        ...
        st.markdown(f"""
- **수입 건강기능식품(CSV)**과 **국내 신고 건강기능식품(오픈 API/동기화 CSV)** 및 비교 분석용 **일반 캔디구미 제품**을 하나로 병합하였습니다.
...
<div class="card">
    ...
    if "부모님" in target_sel:
        ...
        cur_desc = "잦은 야근 and 모니터 주시로 피로한 직장 동료를 위해, 간 건강을 돕는 실리마린(밀크씨슬)과 피로 회복용 활성 비타민 B군, 안구 건조용 루테인을 추천합니다."
